...
const TownshipsData: TownshipData[] = JSON.parse(fs.readFileSync(`${dataProcessingDir}/data-input/townships_data.json`, 'utf8'));
...
export const parseSingleDate = (utcValue: number) => {
...
export function getStandardizedOfficeName(name: string, state: string): string {
  const abbr = US.lookup(state).abbr;
  return (exceptions[abbr] && exceptions[abbr][name]) ? exceptions[abbr][name] : name;
}

export function getTownshipFeaturesForOffice(townshipData: TownshipData): TownshipFeature[] {
...
export function overlapsWithFiscalYear(start: number, end: number, fiscalYear: number, debug = false): boolean {
...
export function overlapsWithFiscalYear2(startYMD: YMD, endYMD: YMD, fiscalYear: number, debug = false): boolean {
...
export function getTownshipFeaturesOnDate(year: number, month: number, day: number, stateTerr?: string): TownshipFeature[] {
  return Townships.features
    .filter(d => {
...
export const getMapPath = (fileName: string): string | null => {
  ...
  if (endYear >= startYear) {
    const mapN = (fileName in (officeMappings as OfficeMappings)) ? (officeMappings as OfficeMappings)[fileName]   // exceptions where it's better to use a different map from the next or a previous map than the current year
      : (shortenedFileName in (officeMappings as OfficeMappings)) ? (officeMappings as OfficeMappings)[shortenedFileName]   // exceptions where it's better to use a different map from the next or a previous map than the current year
...
export const project = (district: TownshipFeature) => {
...
export const getOfficeNameFromStub = (stub: string, stateAbbr: string): string => {
...
export function getStateAbbr(officeName: string, year:number = null): string {
