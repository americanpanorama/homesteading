...
import type { TimelinePlaceData } from '../index.d';
...
const nationalTimeline = readJson<TimelinePlaceData[]>(NATIONAL_TIMELINE_PATH);
const stateTimelineFiles = fs.readdirSync(TIMELINE_ROOT)
  .filter(fileName => fileName.endsWith('.json') && fileName !== 'national.json')
  .map(fileName => fileName.replace(/\.json$/, ''));

const statesTerrsData: StateTerritoryData[] = stateTimelineFiles.map((stateStub) => {
  const stateSummary = nationalTimeline.find(place => place.type === 'stateOrTerritory' && place.abbr === stateStub);
  const stateTimeline = readJson<TimelinePlaceData[]>(path.join(TIMELINE_ROOT, `${stateStub}.json`));
