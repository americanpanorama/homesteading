import React, { useState, useEffect } from 'react';
import { useLocation } from 'react-router-dom';
import Map from './Map/Index';
import * as Styled from './styled';

const Home = () => {

    const { pathname } = useLocation();

    const isMapPath = pathname.startsWith('/map');

        // this keeps track of the most recent map viewed so if the users checks out the intro or the about page they can return to the map they were looking at most recently
        const [mapPath, setMapPath] = useState((isMapPath) ? pathname : '/map');
        useEffect(() => {
            if (isMapPath) {
                setMapPath(pathname);
            }
        }, [pathname]);
  
    return (
      <Styled.Home>
        <Styled.Title>
          <Styled.Acquisition>Land Acquisition</Styled.Acquisition>
          <Styled.And>&</Styled.And>
          <Styled.Dispossession>Dispossession</Styled.Dispossession>
        </Styled.Title>
        <Styled.Subtitle>Mapping the Homestead Act, 1863&#8288;-&#8288;1912</Styled.Subtitle>
        <Styled.Description>In the half century after the Homestead Act of 1862, hundreds of thousands of settlers used federal law to claim private ownership of land in the American West. Those lands became available for distribution only through the United States's long campaign to coerce, dispossess, and remove Native peoples from their homelands. This map visualizes that process: the transfer of land from Native nations to the federal public domain and then into private hands.</Styled.Description>
        
        <Styled.Explore>
          <Map />
        </Styled.Explore>
        <Styled.ExploreButton to={mapPath}>
          <svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
            fill="currentColor"
            style={{ marginRight: "8px" }}
          >
            <path d="M10 2a8 8 0 105.293 14.293l4.707 4.707 1.414-1.414-4.707-4.707A8 8 0 0010 2zm0 2a6 6 0 110 12 6 6 0 010-12z" />
          </svg>
          Explore the Map
        </Styled.ExploreButton>
      </Styled.Home>
  
    )
  }
  
  export default Home;
