...
import { Dimensions, ClaimsAndPatentsAcresType } from '../../../index.d';
...
import TileLayersStyles from './styled';

const TileLayers = () => {
  const { useState, useEffect, useRef, useContext } = React;
  const { center, rotation, scale: mapScale } = useMapView();
  ...
  const calculateZ: CalculateZ = (scale) => {
    ...
    return 1; // default to 1 if something goes wrong; this is the lowest zoom level and will just return the one tile for the whole country
  }
  ...
  const getTiles = (offices: ProjectedTownship[], scale: number, center: [number, number]): TileData[] => {
  ...
  const [tiles, setTiles] = useState<TileData[]>(getTiles(offices, scale, center));
  ...
  useEffect(() => {
    setTiles(getTiles(offices, mapScale, center));
  }, [mapScale, center[0], center[1]]);

  // when there's a new zoom, add the tile layer
  useEffect(() => {
    if (calculateZ(mapScale) > calculateZ(scale)) {
      setOverTiles(getTiles(offices, mapScale, center));
      if (animatingYearStatus.current === 'finished') {
        animatingYearStatus.current = 'finishing';
      }
    } else if (calculateZ(mapScale) < calculateZ(scale)) {
      setUnderTiles(getTiles(offices, mapScale, center));
      if (animatingYearStatus.current === 'finished') {
        animatingYearStatus.current = 'finishing';
      }
    }
  ...
  useEffect(() => {
    ...
    const nextTileSet = getTiles(offices, mapScale, center);
    ...
    const currentTiles = tiles
      .map(d => ({
        ...d,
        opacity: (nextTileSet.map(d1 => d1.tile_id).includes(d.tile_id)) ? nextTileSet.find(d1 => d1.tile_id === d.tile_id)?.opacity || 0 : 0,
      }));
  ...
  useEffect(() => {
    if (animatingYearStatus.current === 'started' && underTiles.length > 0) {
      ...
      const newTiles = underTiles.map(d => ({
          ...d,
          opacity: (nextTileSet.find(d1 => d1.tile_id === d.tile_id)) ? nextTileSet.find(d1 => d1.tile_id === d.tile_id)?.opacity || 0 : 1,
        }));
      setUnderTiles(newTiles);
      animatingYearStatus.current = 'finishing';
    }
  ...
  // clean up after the animation above completes and scale is set to mapScale; over- or underTiles become the "main" tiles and are emptied
  useEffect(() => {
    if (mapScale === scale && animatingYearStatus.current == 'finished') {
  ...
  return (
    <>
      ...
      >
        <TileLayer
          tiles={underTiles}
          scale={getTileScale(scale, mapScale) / scale}
          key='undertiles'
        />
