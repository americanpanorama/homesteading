...
const FullStateDistrict = ({ abbr, projectedTownship, fill, scale }: { abbr: 'IL' | 'IN' | 'OH' | 'MS', projectedTownship: ProjectedTownship | undefined, fill: string, scale: number }) => {
      ...
      <g>
        {features.filter(d => d).map((d) => (
            <path
              d={d}
              stroke='black'
              strokeWidth={getScaledStrokeWidth(scale, 0.35, 0.08, 0.24)}
              strokeOpacity={opacity}
              fill={colors.fullStateDistrictFillColor}
